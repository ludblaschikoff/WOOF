...
#USAGE: bash path_to_script/run_WGS_script_preprocessing_DOGPT49_81.sh path/to/read1:path/to/read2 trimqual minreadlength threads path/to/output samplename
...
if [ $pair = 'true' ]; then
	...
	cutadapt --pair-filter=any -b "GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG" -q "$trimqual","$trimqual" -j "$threads" -m "$minreadlength" --max-n 0 -o "$base_output"/preprocessing/cutadapt/"$base_name"_trimmed.fq.gz "$read1"
fi
