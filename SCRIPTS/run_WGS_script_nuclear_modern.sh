...
5th argument is the minimum snp coverage. example 7.0
6th argument is the minimum snp quality. example 30.0
7th argument is the minimum genotype quality. example 20.0
...
samtools sort "$base_name"_no_dups.bam > "$base_name"_no_dups_sorted.bam
samtools index "$base_name"_no_dups_sorted.bam

rm $base_name'_sorted.bam' $base_name'_sorted.bam.bai' $base_name'_RG.bam' $base_name'.sam'

qualimap bamqc -nt "$threads" -c -bam "$base_name"_no_dups_sorted.bam --java-mem-size=32G
...
gatk HaplotypeCaller -R "$reference" -I "$base_name"_no_dups_sorted.bam --bam-output "$base_name"_GATK_out.bam -O "$base_output"/variant_calling/"$base_name".vcf
