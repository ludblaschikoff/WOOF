...
geno=0.1	#Missingness per SNP
mind=0.25	#Missingness per individual
maf=0.01	#Minor allele frequency
LDr=0.2	#indep-pairwise
hwe=0.001 #Hardy-Weinberg 
...
plink --bfile dog.cleaned.geno"$geno".mind"$mind".maf"$maf" --dog --allow-extra-chr --autosome --double-id --make-pheno temp-cases.txt '*' --allow-no-sex --nonfounders --keep-allele-order --hwe $hwe --make-bed --out dog.cleaned.geno"$geno".mind"$mind".maf"$maf".hwe"$hwe" 

plink --bfile dog.cleaned.geno"$geno".mind"$mind".maf"$maf".hwe"$hwe" --dog --allow-extra-chr --autosome --double-id --make-pheno temp-cases.txt '*' --allow-no-sex --nonfounders --keep-allele-order --indep-pairwise 50 5 $LDr --make-bed --out dog.cleaned.geno"$geno".mind"$mind".maf"$maf".hwe"$hwe".LDr"$LDr"

plink --bfile dog.cleaned.geno"$geno".mind"$mind".maf"$maf".hwe"$hwe".LDr"$LDr" --dog --allow-extra-chr --autosome --double-id --make-pheno temp-cases.txt '*' --allow-no-sex --nonfounders --keep-allele-order --extract dog.cleaned.geno"$geno".mind"$mind".maf"$maf".hwe"$hwe".LDr"$LDr".prune.in --make-bed --out dog.pruned

# Perform Relatedness. 

plink2 --dog --threads 10 --memory 300000 --bfile dog.pruned --make-king-table --out dog.kinship.table 

plink2 --dog --threads 10 --memory 300000 --bfile dog.pruned --king-cutoff .177 --make-bed --out dog.kingship_clean 

plink --bfile dog.kingship_clean --recode --keep-allele-order --het --make-bed --allow-no-sex --dog --out dog.kingship_clean_recode


rm temp-cases.txt #remove temporary file

# Create missing genotype statistic individual and loci after filtering
plink --bfile dog.cleaned.geno"$geno".mind"$mind" --dog --allow-extra-chr --autosome --missing --out missing_genotype
